function dataUpdating_breaks() {
  ...
  var sheetBreaks = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(
    "iCX Broken Approvals"
  ); // write sheet name
  var startDate = "01/02/2023";
  ...
  try {
    ...
    var ids = sheetBreaks
      .getRange(1, 1, sheetBreaks.getLastRow(), 1)
      .getValues();
    var ids = ids.flat(1);
    var newRows = [];
        ...
        if (index < 0) {
          ...
          newRows.push([
            ...
            data[i].date_matched != null
              ...
              : "",
            data[i].date_approval_broken != null
              ? data[i].date_approval_broken.toString().substring(0, 10)
              : "",
          ]);
        } else {
          Logger.log("old");
